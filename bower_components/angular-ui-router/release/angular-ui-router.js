...
var PathUtils = /** @class */ (function () {
     ...
     *
     * On each [[PathNode]], creates ViewConfig objects from the public: property of the node's state
     */
...
var Transition = /** @class */ (function () {
     ...
     *
     * Each state can define one or more public (template/controller), which are encapsulated as `ViewConfig` objects.
     * This method fetches the `ViewConfigs` for a given path in the Transition (e.g., "to" or "entering").
     *
     * @param pathname the name of the path to fetch public for:
...
var StateBuilder = /** @class */ (function () {
    function StateBuilder(matcher, urlMatcherFactory) {
        ...
        this.builders = {
            ...
            // Each framework-specific ui-router implementation should define its own `public` builder
            // e.g., src/ng1/statebuilders/public.ts
            views: [],
 ...
 * with view configs (from the state declaration objects: [[StateDeclaration.public]]).
 *
 * - After a successful Transition, the public from the newly entered states are activated via [[activateViewConfig]].
 *   The public from exited states are deactivated via [[deactivateViewConfig]].
...
var ViewService = /** @class */ (function () {
    ...
    ViewService.prototype.sync = function () {
        ...
        // The depth is the nesting depth of ui-public (based on FQN; times 10,000)
        ...
        var configureUIView = function (_a) {
            ...
            // If a parent ui-view is reconfigured, it could destroy child ui-public.
            // Before configuring a child ui-view, make sure it's still in the active uiViews array.
            if (_this._uiViews.indexOf(uiView) !== -1)
                uiView.configUpdated(viewConfig);
        };
        // Sort public by FQN and state depth. Process uiviews nearest the root first.
    ...
    /**
     * Returns the list of public currently available on the page, by fully-qualified name.
     *
    ...
    /**
     * Returns the list of public on the page containing loaded content.
     *
     * @return {Array} Returns an array of fully-qualified view names.
     */
    ...
    /**
     * Normalizes a view's name from a state.public configuration block.
     *
     * This should be used by a framework implementation to calculate the values for
     * [[_ViewDeclaration.$uiViewName]] and [[_ViewDeclaration.$uiViewContextAnchor]].
     *
     * @param context the context object (state declaration) that the view belongs to
     * @param rawViewName the name of the view, as declared in the [[StateDeclaration.public]]
     *
...
/**
 * A [[TransitionHookFn]] which waits for the public to load
 *
 * Registered using `transitionService.onStart({}, loadEnteringViews);`
 *
 * Allows the public to do async work in [[ViewConfig.load]] before the transition continues.
...
/**
 * A [[TransitionHookFn]] which activates the new public when a transition is successful.
 *
 * Registered using `transitionService.onSuccess({}, activateViews);`
 *
 * After a transition is complete, this hook deactivates the old public from the previous state,
 * and activates the new public from the destination state.
 *
 * See [[ViewService]]
 */
...
/**
 * This is a [[StateBuilder.builder]] function for angular1 `public`.
 *
 * When the [[StateBuilder]] builds a [[StateObject]] object from a raw [[StateDeclaration]], this builder
 * handles the `public` property with logic specific to @uirouter/angularjs (ng1).
 *
 * If no `public: {}` property exists on the [[StateDeclaration]], then it creates the `public` object
...
function ng1ViewsBuilder(state) {
    ...
    // Do not allow a state to have both state-level props and also a `public: {}` property.
    // A state without a `public: {}` property can declare properties for the `$default` view as properties of the state.
    // However, the `$default` approach should not be mixed with a separate `public: ` block.
    if (isDefined(state.views) && hasAnyKey(allViewKeys, state)) {
        throw new Error("State '" + state.name + "' has a 'public' object. " +
            "It cannot also have \"view properties\" at the state level.  " +
            "Move the following properties into a view (in the 'public' object): " +
    ...
    forEach(viewsObject, function (config, name) {
        // Account for public: { "": { template... } }
        name = name || "$default";
        // Account for public: { header: "headerComponent" }
        if (isString(config))
...
function $uiRouter($locationProvider) {
    ...
    // Apply ng1 specific StateBuilder code for `public`, `resolve`, and `onExit/Retain/Enter` properties
 ...
 *   The name should be unique amongst the other public in the same state.
 *   You can have public of the same name that live in different states.
 *   The ui-view can be targeted in a View using the name ([[Ng1StateDeclaration.public]]).
 *
 ...
 * [[Ng1StateDeclaration.public]] config property, by name, in this case an empty name:
 *
 * ```js
 * $stateProvider.state("home", {
 *   public: {
